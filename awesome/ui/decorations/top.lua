...
local get_titlebar = function(c, height)
  ...
  local ci = function(width, height)
    return function(cr)
      gears.shape.transform(gears.shape.cross):rotate_at(width / 2, height / 2, math.pi / 4)(cr, width, height, 2)
    end
  end

  local close = create_title_button(c, beautiful.xcolor1, beautiful.xcolor0 .. "55", ci(12, 12))
