...
local function fair_split(length, shares)
  ...
  for i = 1, #shares do
    if ret[i] == nil then
      ...
      ret[i] = remaining == 1 and length - acc_ret
        or math.floor((length - sum_adj) / sum_weight * acc_weight + acc_adj - acc_ret + 0.5)
...
local function areas_from_command(command, workarea, minimum)
  ...
  local function handle_op(method)
    ...
    if method == "h" or method == "v" then
      ...
      for i = 1, #args do
        ...
        if arg[2] == 0 and arg[3] then
          arg[2], arg[3] = -arg[3], nil
        end
        shares[i] = arg
      end
    ...
    elseif method == "w" or method == "d" then
      ...
      if method == "w" then
        ...
        if x_shares_count < 1 then
          x_shares_count = 1
        end
        if y_shares_count < 1 then
          y_shares_count = 1
        end
        ...
        for i = 1, x_shares_count do
          x_shares[i] = { 1 }
        end
        for i = 1, y_shares_count do
          y_shares[i] = { 1 }
        end
      ...
      for y_index = 1, #a.split.y_shares do
        for x_index = 1, #a.split.x_shares do
          ...
          if x_index == #a.split.x_shares then
            r.br = a.br
          else
            r.br = false
          end
          ...
          if y_index == #a.split.y_shares then
            r.bd = a.bd
          else
            r.bd = false
          end
      ...
      for i = m_start, #args - 1, 2 do
        ...
        if x + w > #x_shares then
          w = #x_shares - x
        end
        if y + h > #y_shares then
          h = #y_shares - y
        end
        ...
        local r = {
          bu = children[start_index].bu,
          bl = children[start_index].bl,
          bd = children[end_index].bd,
          br = children[end_index].br,
    ...
    if a.split then
      ...
      for _, c in ipairs(a.split.children) do
        ...
        if c.split then
          ...
          x_minimum, y_minimum = minimum, minimum
        end
  ...
  for i = 1, #closed_areas do
    if
      closed_areas[i].x + closed_areas[i].width > root.x + orig_width
      or closed_areas[i].y + closed_areas[i].height > root.y + orig_height
    then
  ...
  for i = 1, #open_areas do
    if
      open_areas[i].x + open_areas[i].width > root.x + orig_width
      or open_areas[i].y + open_areas[i].height > root.y + orig_height
    then
      ...
      elseif method == "d" or method == "w" then
        ...
        for _, c in ipairs(a.split.children) do
          if type(c.parent_x_shares) == "table" then
            ...
            m = m
              .. tostring(c.parent_x_shares[#c.parent_x_shares] - c.parent_x_shares[1] + 1)
              .. ","
