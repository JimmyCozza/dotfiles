...
function module.start(c, exit_keys)
  local tablist_font_desc = beautiful.get_merged_font(beautiful.font, dpi(10))
  ...
  if (c ~= nil and c.floating) or layout.machi_get_instance_data == nil then
    return
  end
  ...
  local function set_selected_area(area)
    ...
    if area then
      traverse_x = max(
        areas[area].x + traverse_radius,
        min(areas[area].x + areas[area].width - traverse_radius, traverse_x)
      )
      traverse_y = max(
        areas[area].y + traverse_radius,
        min(areas[area].y + areas[area].height - traverse_radius, traverse_y)
      )
    end
  end
  ...
  local function selected_area()
    if selected_area_ == nil then
      ...
      for i, a in ipairs(areas) do
        if a.habitable then
          local dis = math.abs(a.x + traverse_radius - traverse_x)
            + math.abs(a.x + a.width - traverse_radius - traverse_x)
            - a.width
            + math.abs(a.y + traverse_radius - traverse_y)
            + math.abs(a.y + a.height - traverse_radius - traverse_y)
            - a.height
            + traverse_radius * 4
    ...
    if tablist == nil then
      ...
      for _, tc in ipairs(screen.tiled_clients) do
        if not (tc.floating or tc.immobilized) then
          if
            areas[active_area].x <= tc.x + tc.width + tc.border_width * 2
            and tc.x <= areas[active_area].x + areas[active_area].width
            and areas[active_area].y <= tc.y + tc.height + tc.border_width * 2
            and tc.y <= areas[active_area].y + areas[active_area].height
          then
  ...
  local function draw_info(context, cr, width, height)
    ...
    if #tablist > 0 then
      ...
      cr:rectangle(
        a.x + (a.width - list_width) / 2 - start_x,
        a.y + (a.height - list_height) / 2 - start_y,
        list_width,
        list_height
      )
      ...
      for index, tc in ipairs(tablist) do
        ...
        if index == tablist_index then
          cr:rectangle(
            x_offset - ext.width / 2 - vpadding / 2,
            y_offset - vpadding / 2,
            ext.width + vpadding,
            ext.height + vpadding
          )
    ...
    cr:rectangle(
      traverse_x - start_x - traverse_radius,
      traverse_y - start_y - traverse_radius,
      traverse_radius * 2,
      traverse_radius * 2
    )
  ...
  local function handle_key(mod, key, event)
    ...
    elseif key == "Up" or key == "Down" or key == "Left" or key == "Right" then
      ...
      if c and (shift or ctrl) then
        if shift then
          if current_area == nil or areas[current_area].x ~= c.x or areas[current_area].y ~= c.y then
            traverse_x = c.x + traverse_radius
            traverse_y = c.y + traverse_radius
        ...
        elseif ctrl then
          ...
          if
            current_area == nil
            or areas[current_area].x + areas[current_area].width ~= ex
            or areas[current_area].y + areas[current_area].height ~= ey
          then
      ...
      for i, a in ipairs(areas) do
        if not a.habitable then
          goto continue
        end

        local v
        if key == "Up" then
          if a.x < traverse_x + threshold and traverse_x < a.x + a.width + threshold then
        ...
        elseif key == "Left" then
          if a.y < traverse_y + threshold and traverse_y < a.y + a.height + threshold then
      ...
      if choice ~= nil then
        ...
        if c and ctrl and cd[c].draft ~= false then
          ...
          if shift then
            lu = choice
            if areas[rd].x + areas[rd].width <= areas[lu].x or areas[rd].y + areas[rd].height <= areas[lu].y then
              rd = nil
            end
          else
            rd = choice
    ...
    elseif key == "q" or key == "Prior" then
      ...
      if #parent_stack == 0 or parent_stack[#parent_stack] ~= current_area then
    ...
    elseif key == "/" then
      ...
      local prefix, suffix = machi.engine.areas_to_command(areas, false):match "(.*)|(.*)"
      ...
      gtimer.delayed_call(function()
        layout.machi_editor.start_interactive(screen, {
          workarea = workarea,
  ...
  kg = awful.keygrabber.run(function(...)
