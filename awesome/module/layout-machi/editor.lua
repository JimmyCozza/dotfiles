...
function module.create(data)
  ...
  local function start_interactive(screen, embed_args)
    ...
    local function set_cmd(cmd)
      local new_closed_areas, new_open_areas, new_pending_op = machi_engine.areas_from_command(cmd, {
        x = workarea.x + gap,
        y = workarea.y + gap,
      ...
      if new_closed_areas then
        closed_areas, open_areas, pending_op = new_closed_areas, new_open_areas, new_pending_op
        current_cmd = cmd

        if embed_args then
          current_info = embed_args.cmd_prefix .. "[" .. current_cmd .. "]" .. embed_args.cmd_suffix
        else
          current_info = cmd
        end
    ...
    local function get_final_cmd()
      ...
      if embed_args then
        final_cmd = embed_args.cmd_prefix .. machi_engine.areas_to_command(closed_areas, true) .. embed_args.cmd_suffix
      end
      return final_cmd
    end
    ...
    kg = awful.keygrabber.run(function(mod, key, event)
  ...
  local function run_cmd(cmd, screen, tag)
    local gap = tag.gap
    local areas, closed = machi_engine.areas_from_command(cmd, {
  ...
  function adjust_shares(c, axis, adj)
    ...
    local tag = screen.selected_tag
    local layout = tag.layout
    if not layout.machi_get_instance_data then
      return
    end
