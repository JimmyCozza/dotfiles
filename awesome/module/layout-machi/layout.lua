  ...
  for i, a in ipairs(areas) do
    if a.habitable then
      if lu == nil or (a.x + a.width > areas[lu].x and a.y + a.height > areas[lu].y) then
        if
          rd == nil
          or distance(x, y, a.x + a.width, a.y + a.height)
            < distance(x, y, areas[rd].x + areas[rd].width, areas[rd].y + areas[rd].height)
        then
          rd = i
        end
      end
...
local function get_machi_tag_string(tag)
  if tag.machi_tag_string == nil then
    tag.machi_tag_string = tostring(tag.screen.geometry.width)
      .. "x"
      .. tostring(tag.screen.geometry.height)
      .. "+"
      .. tostring(tag.screen.geometry.x)
      .. "+"
      .. tostring(tag.screen.geometry.y)
      .. "+"
      .. tag.name
  end
...
function module.create(args_or_name, editor, default_cmd)
  ...
  local function get_instance_data(screen, tag)
    ...
    local key = tostring(workarea.width)
      .. "x"
      .. tostring(workarea.height)
      .. "+"
      .. tostring(workarea.x)
      .. "+"
      .. tostring(workarea.y)
      ...
      if c.floating or c.immobilized then
        ...
        if in_draft ~= false then
          if
            ...
            and cd[c].lu <= #areas
            and cd[c].rd <= #areas
            and areas[cd[c].lu].habitable
            and areas[cd[c].rd].habitable
          then
            if
              areas[cd[c].lu].x == geo.x
              and areas[cd[c].lu].y == geo.y
              and areas[cd[c].rd].x + areas[cd[c].rd].width == geo.x + geo.width
              and areas[cd[c].rd].y + areas[cd[c].rd].height == geo.y + geo.height
            then
              skip = true
            end
          end
          ...
          if
            cd[c].area ~= nil
            and cd[c].area <= #areas
            and areas[cd[c].area].habitable
            and areas[cd[c].area].layout == nil
            and areas[cd[c].area].x == geo.x
            and areas[cd[c].area].y == geo.y
            and areas[cd[c].area].width == geo.width
            and areas[cd[c].area].height == geo.height
          then
            skip = true
          else
            log(DEBUG, "Compute areas for " .. (c.name or ("<untitled:" .. tostring(c) .. ">")))
        ...
        if skip then
          if
            geo.x ~= c.x
            or geo.y ~= c.y
            or geo.width ~= c.width + c.border_width * 2
            or geo.height ~= c.height + c.border_width * 2
          then
            p.geometries[c] = {}
    ...
    if context == "mouse.move" then
      ...
      if in_draft then
        ...
        if lu ~= nil then
          ...
          if
            rd ~= nil
            and not module.allowing_shrinking_by_mouse_moving
            and (
              areas[rd].x + areas[rd].width - areas[lu].x < c.full_width_before_move
              or areas[rd].y + areas[rd].height - areas[lu].y < c.full_height_before_move
            )
          then
