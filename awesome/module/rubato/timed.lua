...
local function simulate_easing(pos, duration, intro, intro_e, outro, outro_e, m, b, dt)
  ...
  local key = string.format("%f %f %f %s %f %s %s %s", pos, duration, intro, intro_e, outro, outro_e, m, b)
...
local function timed(args)
  ...
  elseif not args.outro then
    ...
    obj.outro = args.outro
  end

  --assert that these values are valid
  assert(
    obj.intro + obj.outro <= obj.duration or obj.prop_intro,
    "Intro and Outro must be less than or equal to total duration"
  )
  assert(
    obj.intro + obj.outro <= 1 or not obj.prop_intro,
  ...
  timer:connect_signal("timeout", function()
    ...
    if obj.duration - time < dt / 2 then
      ...
      if obj.awestore_compat then
        obj.ended:fire(obj.pos, time, dx)
      end
  ...
  local function set(target_new)
    ...
    if not obj.rapid_set and obj._props.target == target_new then
      return
    end
