...
M.config = function()
  ...
  function _G.__fterm_lazydocker()
    if is_installed "lazydocker" ~= true then
      print "Please install lazydocker."
      return
    end
    lazydocker:toggle()
  end
  ...
  O.plugin.which_key.mappings["dd"] = "LazyDocker"
  vim.api.nvim_set_keymap("n", "<A-i>", "<CMD>lua require('FTerm').toggle()<CR>", { noremap = true, silent = true })
  vim.api.nvim_set_keymap("n", "<leader>dd", "<CMD>lua _G.__fterm_lazydocker()<CR>", { noremap = true, silent = true })
